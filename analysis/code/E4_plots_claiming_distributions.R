...
plot1 <- panel[!is.na(claim_haz) & year_month >= 2012 & year_month <= 2018.5] %>% 
  .[points_norm %in% c(-4:3)] %>% 
  .[, .(avg = mean(claim_haz, na.rm = T)), by = .(year_month, points_norm)] %>%
  .[, cat_linetype := ifelse(points_norm < 0, 'dashed', 'solid')] %>% 
  ggplot(aes(x = year_month, y = avg, color = factor(points_norm)))+
...
plot2 <- panel[!is.na(claim_haz) & year_month >= 2012 & year_month < 2018.5 & points_norm %in% c(-15:15)] %>% 
  ...
  # .[, dist_reform := (year_month - 2015.25)*4] %>% 
  .[, .(avg = mean(claim_haz, na.rm = T)), by = .(dist_reform_quarters, points_norm_aux)] %>%
  .[, cat_linetype := ifelse(points_norm_aux %in% c('[-15,-7]', '[-6,-3]', '[-2,-1]'), 
                             'dashed', 'solid')] %>% 
  ggplot(aes(x = dist_reform_quarters, y = avg, color = factor(points_norm_aux)))+
