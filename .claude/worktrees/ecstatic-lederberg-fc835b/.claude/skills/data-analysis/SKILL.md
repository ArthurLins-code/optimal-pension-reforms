...
- **Use `saveRDS()`** for every computed object — downstream scripts or manuscripts may need them
...
- `.claude/rules/content-invariants.md` — [INV-1, INV-2, INV-3 applicable]
...
3. Set seed once at top in YYYYMMDD format (per `r-code-conventions.md`), e.g. `set.seed(20260415)` (INV-1)
...
set.seed(20260415)  # YYYYMMDD per r-code-conventions.md (INV-1)
