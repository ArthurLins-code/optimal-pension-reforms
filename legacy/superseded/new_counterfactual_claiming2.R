stop("SUPERSEDED — not part of the current workflow. Canonical replacement: new_counterfactual_claiming3_pure.R. Archived 2026-06-23 (usage audit); see legacy/superseded/README.md.")
...
sum(dt_final[t >= 0]$claims) # 606,605
sum(dt_final[t >= 0]$claims_c) # 597,328
...
ggplot()+
  geom_line(data = dt_freq[t == 10], aes(x = p, y = value, color = factor(variable)))+
  geom_point(data = dt_freq[t == 10], aes(x = p, y = value, color = factor(variable)))+
  geom_vline(xintercept = 0)+
  geom_hline(yintercept = 0)+
  theme_classic()
...
for (y in seq(0,13,1)) {
  ...
  list_plots_count[[paste0(y)]] <- dt_freq[t == y] %>% 
    .[,.(p, variable, value)] %>% 
    ggplot(aes(x = p))+
    geom_vline(xintercept = 0, linetype = 'longdash', linewidth = 0.3, color = 'firebrick')+
    geom_hline(yintercept = 0, linetype = 'longdash', linewidth = 0.3, color = 'firebrick')+
    geom_line(aes(y = value, color = factor(variable)))+
    geom_point(aes(y = value, color = factor(variable)), shape = 17, size = 0.8)+
    scale_color_manual(values = c('claims'='dodgerblue4','claims_c'='maroon3'), 
                       labels = c('claims'=paste0(cat),'claims_c'='Count. (DD)'))+
    scale_fill_manual(values = c('claims'='dodgerblue4','claims_c'='maroon3'), 
...
plot_all <- dt_freq_all %>% 
  ...
  geom_point(aes(y = value, color = factor(variable)), shape = 17, size = 0.8)+
  scale_color_manual(values = c('claims'='dodgerblue4','claims_c'='maroon3'), 
                     labels = c('claims'='Actual','claims_c'='Count. (DD)'))+
  scale_fill_manual(values = c('claims'='dodgerblue4','claims_c'='maroon3'), 
                    labels = c('1'='Actual','2'='Counterfactual'))+
  scale_x_continuous(breaks = seq(-15,15,5), minor_breaks = seq(-30,30,1),
                     guide = guide_axis(minor.ticks = TRUE))+
...
fwrite(dt_final,"output/F/new_counterfactual_claim_counts.csv")
