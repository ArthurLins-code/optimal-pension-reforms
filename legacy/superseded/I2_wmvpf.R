...
dt_gab <- fread('working/D1_cross_section.csv.gz')
...
panel <- left_join(panel, dt_gab[,.(indiv, benef_size)], by = 'indiv')
...
aux0 <- dt_gab[dist_reform %in% 0:12, .(avg_benefits = mean(benef_size, na.rm = T)*3), by = .(dist_reform, points_norm)] %>% 
...
##########################################################
# Adapting this code for pure reforms
#########################################################
...
aux_avg_pure_benefits_<- DT[,. (dist_reform,points_norm,avg_ben_bL=(avg_reform_benefits_pre_reform_choices_bL*3),avg_ben_bS=(avg_reform_benefits_pre_reform_choices_bS*3))]

aux6 <- DT[dist_reform >= 0 & dist_reform <= 12,.(dist_reform, points_norm, delta_freq_perc_bL = (freq_count-dL)/dL,delta_freq_perc_bS = (freq_count-dS)/dS)] %>% 
  left_join(n_claims, by = c('points_norm','dist_reform')) %>% 
  .[, c("num_claims_count_bL","num_claims_count_bS"):=.( num_claims*(1+delta_freq_perc_bL),
