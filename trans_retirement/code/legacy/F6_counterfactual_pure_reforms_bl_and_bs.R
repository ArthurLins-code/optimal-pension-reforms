...
rel_freq <- function(data, variable, bandwidth, i) {
  ...
  rel_freq <- freq_table / sum(freq_table) # to work with frequencies, remove this /sum(freq_table)
...
fn_distribution <- function(df) {
  ...
  distribution[, hazard := freq/(1-cum_lag)] #to work with frequencies, change 1 for the total number of counts
